...
export function Sidebar() {
  ...
  return (
    <div className="w-64 border-r border-border bg-sidebar h-screen flex flex-col fixed left-0 top-0 text-sidebar-foreground">
      ...
      <nav className="flex-1 py-6 px-4 flex flex-col gap-0.5 overflow-y-auto">
            ...
            >
              <Link href={item.href}>
                <motion.div
                  ...
                  className={cn(
                    "relative flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium cursor-pointer select-none",
                    isActive
                ...
                >
                  ...
                  {!isActive && (
                    <motion.div
                      className="absolute inset-0 rounded-md bg-sidebar-accent/0 group-hover:bg-sidebar-accent/40"
                    />
                  )}
                  <motion.span
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isAdmin2 && (user?.subroles?.length ?? 0) > 1 && (
              <div className="text-[10px] text-muted-foreground/60 mt-0.5">
                +{(user?.subroles?.length ?? 0) - 1} autre(s)
              </div>
            )}
          </div>
        </motion.div>
